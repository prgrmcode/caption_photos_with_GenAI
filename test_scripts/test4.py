    ...
    with gr.Blocks() as demo:
        ...
        with gr.Tab("Caption from Folder"):
            ...
            def folder_generate_captions(folder_files):
                if not folder_files:
                    return (
                        [gr.update(visible=False) for _ in folder_image_components],
                        [gr.update(visible=False) for _ in folder_caption_components],
                ...
                for i in range(MAX_IMAGES):
                    if i < len(captions):
                        image_updates.append(
                            gr.update(value=captions[i]["image"], visible=True)
                        )
                        caption_updates.append(
                            gr.update(
                                value=captions[i]["modified_caption"], visible=True
                            )
                        )
                    else:
                        image_updates.append(gr.update(visible=False))
                        caption_updates.append(gr.update(visible=False))
                return (
                    image_updates
                    + caption_updates
                    + ["### Images and captions loaded successfully."]
            ...
            def folder_save_captions_fn(*args):
                num_images = len(folder_image_components)
                save_path = args[-1]
                image_values = args[:num_images]
                caption_values = args[num_images:-1]

                captions = []
                for img_value, cap_value in zip(image_values, caption_values):
                        ...
                        captions.append(
                            {
                                "image": image,
                                "modified_caption": caption,
                                "original_caption": caption,  # Adjust if tracking original captions separately
                            }
                        )
