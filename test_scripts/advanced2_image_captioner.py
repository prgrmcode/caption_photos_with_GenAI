            ...
            with url_gallery:
                for idx in range(MAX_IMAGES):
                    with gr.Row(visible=False) as row:
                        # image = gr.Image(label=f"Image {idx + 1}", interactive=False)
                        image = gr.Image(
                            label=f"Image {idx+1}", height=150, visible=False
                        )
                        # caption = gr.Textbox(label="Edit Caption", interactive=True)
                        caption = gr.Textbox(
